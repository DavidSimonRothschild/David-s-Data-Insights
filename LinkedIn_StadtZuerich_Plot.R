...
# Daten vorbereiten - nur Zeilen mit Stadtkreis-Namen (nicht "Durchschnitt")
data_clean <- data %>%
  filter(!is.na(Eigentumsobjekte_Prozent) & Durchschnitt != "Ja" & Durchschnitt != "Nein")

# Funktion für einzelne Plots
create_city_plot <- function(data, y_var, y_label, title_suffix, color, filename_prefix) {
  
  # Korrelation berechnen
  correlation <- cor(data[[y_var]], data$Eigentumsobjekte_Prozent, use = "complete.obs")
  ...
  # Plot erstellen
  p <- ggplot(data, aes_string(x = "Eigentumsobjekte_Prozent", y = y_var)) +
    # Punkte
    geom_point(size = 4, color = color, alpha = 0.8) +
    
    # Regressionslinie
    geom_smooth(method = "lm", se = TRUE, color = color, fill = color, alpha = 0.2) +
    ...
    labs(
      ...
      subtitle = paste0("Zusammenhang auf Stadtkreis-Ebene\n",
...
for(abstimmung in c("Ja_Prozent_Eigenmietwert", "Ja_Prozent_Untermiete", "Ja_Prozent_Eigenbedarf")) {
  ...
  correlation <- cor(data_clean[[abstimmung]], data_clean$Eigentumsobjekte_Prozent, use = "complete.obs")
