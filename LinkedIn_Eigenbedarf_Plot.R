...
data_clean <- data %>%
  filter(!is.na(Ja_Prozent_Eigenbedarf) & !is.na(Eigentumsobjekte_Prozent))

# Korrelation berechnen
correlation <- cor(data_clean$Eigentumsobjekte_Prozent, 
                   data_clean$Ja_Prozent_Eigenbedarf, 
                   use = "complete.obs")

# Lineare Regression für R²
model <- lm(Ja_Prozent_Eigenbedarf ~ Eigentumsobjekte_Prozent, data = data_clean)
r_squared <- summary(model)$r.squared

# Hauptplot erstellen
p <- ggplot(data_clean, aes(x = Eigentumsobjekte_Prozent, y = Ja_Prozent_Eigenbedarf)) +
  # Punkte mit Bezirk-Färbung
  geom_point(aes(color = Bezirk), size = 3, alpha = 0.7) +
  
  # Regressionslinie mit Konfidenzintervall
  geom_smooth(method = "lm", se = TRUE, color = "#2E7D32", fill = "#2E7D32", alpha = 0.2) +
  ...
  geom_text_repel(
    data = data_clean %>% 
      filter(Eigentumsobjekte_Prozent > 70 | 
             ...
             Ja_Prozent_Eigenbedarf > 65 |
             Ja_Prozent_Eigenbedarf < 40),
  ...
  labs(
    title = "Wohneigentum und Eigenbedarf-Initiative im Kanton Zürich",
    subtitle = paste0("Zusammenhang zwischen Eigentumsquote und Ja-Anteil zur Eigenbedarf-Initiative\n",
                      "Korrelation: r = ", round(correlation, 3), " | R² = ", round(r_squared, 3),
                      " | Angenommen in ", sum(data_clean$Eigenbedarf_Angenommen), " von ", 
                      nrow(data_clean), " Gemeinden"),
    x = "Wohneigentumsquote (%)",
    y = "Ja-Anteil Eigenbedarf-Initiative (%)",
    color = "Bezirk",
...
ggsave("LinkedIn_Eigenbedarf_Grafik.png", 
       plot = p, 
       width = 12, 
       height = 6.75, 
       dpi = 150,
...
ggsave("LinkedIn_Eigenbedarf_Grafik_square.png", 
       plot = p_square, 
       width = 8, 
       height = 8, 
       dpi = 150,
...
cat("\n=== EIGENBEDARF-INITIATIVE: STATISTISCHE ZUSAMMENFASSUNG ===\n")
...
cat("Gemeinden mit Annahme (>50%):", sum(data_clean$Eigenbedarf_Angenommen), "\n")
cat("Gemeinden mit Ablehnung (<50%):", sum(!data_clean$Eigenbedarf_Angenommen), "\n\n")

# Gemeinden mit höchstem und niedrigstem Ja-Anteil
cat("Top 5 Gemeinden mit höchstem Ja-Anteil:\n")
data_clean %>%
  arrange(desc(Ja_Prozent_Eigenbedarf)) %>%
  head(5) %>%
  select(GEBIET_NAME, Eigentumsobjekte_Prozent, Ja_Prozent_Eigenbedarf) %>%
  print()

cat("\nTop 5 Gemeinden mit niedrigstem Ja-Anteil:\n")
data_clean %>%
  arrange(Ja_Prozent_Eigenbedarf) %>%
  head(5) %>%
  select(GEBIET_NAME, Eigentumsobjekte_Prozent, Ja_Prozent_Eigenbedarf) %>%
  print()
