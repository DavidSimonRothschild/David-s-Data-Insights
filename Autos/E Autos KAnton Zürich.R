...
library("readr")
library("gt")
KANTON_ZUERICH_607 <- read_csv("https://www.web.statistik.zh.ch/ogd/data/KANTON_ZUERICH_607.csv")
...
filtered_df_ev <- KANTON_ZUERICH_607[!(KANTON_ZUERICH_607$GEBIET_NAME %in% entries_to_remove), ]
#KANTON_ZUERICH_607 <- filtered_df_e_Auto
...
summary_data_ev <- filtered_df_ev %>%
...
View(summary_data_ev)

summary_table <- summary_data %>%
  gt()


line_plot_EV <- ggplot(summary_data_ev, aes(x = INDIKATOR_JAHR)) +
  ...
  ggtitle("Entwicklung Elektro Autos")  # Remove the extra x axis name and add a proper title
...
for (INDIKATOR_JAHR in 2002:2022) {
  # Filter the data for the current year
  subset <- filtered_df_e_Auto[filtered_df_e_Auto$INDIKATOR_JAHR == INDIKATOR_JAHR, ]
