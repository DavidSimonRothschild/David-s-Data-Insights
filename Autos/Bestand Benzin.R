...
KANTON_ZUERICH_604 <- read_csv("https://www.web.statistik.zh.ch/ogd/data/KANTON_ZUERICH_604.csv")
...
filtered_df_Benzin <- KANTON_ZUERICH_604[!(KANTON_ZUERICH_604$GEBIET_NAME %in% entries_to_remove), ]
KANTON_ZUERICH_604 <- filtered_df_Benzin
...
summary_data_Benzin <- filtered_df_Benzin %>%
...
View(summary_data_Benzin)

summary_table <- summary_data %>%
  gt()


line_plot_Benzin <- ggplot(summary_data_Benzin, aes(x = INDIKATOR_JAHR)) +
  ...
  ggtitle("Entwicklung Benzin") +
  scale_y_continuous(limits = c(0, 100)) +
  scale_x_continuous(limits = c(2002, 2022))

# Display the line plot for Benzin
print(line_plot_Benzin)
...
for (INDIKATOR_JAHR in 2002:2022) {
  # Filter the data for the current year
  subset <- filtered_df_Benzin[filtered_df_Benzin$INDIKATOR_JAHR == INDIKATOR_JAHR, ]
  
  # Count the number of entries with value 0.0 in INDIKATOR_VALUE for the current year
  count <- sum(subset$INDIKATOR_VALUE < 50.0)
...
for (INDIKATOR_JAHR in 2002:2022) {
  ...
  count_non_zero <- sum(subset$INDIKATOR_VALUE > 50.0)
...
ggplot(data_long, aes(x = as.factor(Year), y = Count, fill = Category)) +
  ...
  labs(
    x = "Jahr",
    y = "ANzahl Gemeinden",
    title = "Entwicklung Bestand Benzin",
    fill = "Category"
  ) +
  scale_fill_manual(values = c("#FFA500", "orange")) +
  theme_minimal()+
  annotate(
