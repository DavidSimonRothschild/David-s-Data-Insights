...
# https://www.web.statistik.zh.ch/ogd/data/KANTON_ZUERICH_605.csv

KANTON_ZUERICH_605 <- read_csv("https://www.web.statistik.zh.ch/ogd/data/KANTON_ZUERICH_605.csv")
...
filtered_df_Diesel <- KANTON_ZUERICH_605[!(KANTON_ZUERICH_605$GEBIET_NAME %in% entries_to_remove), ]
#KANTON_ZUERICH_605 <- filtered_df_Hybrid
...
summary_data <- filtered_df_Diesel %>%
...
line_plot_Diesel <- ggplot(summary_data, aes(x = INDIKATOR_JAHR)) +
...
ggplot(data_long, aes(x = as.factor(Year), y = Count, fill = Category)) +
  ...
  labs(
    x = "Jahr",
    y = "ANzahl Gemeinden",
    title = "Entwicklung Bestand Diesel",
    fill = "Category"
  ) +
  scale_fill_manual(values = c("#FFBF00","orange")) +
  theme_minimal()+
  annotate(
