...
N_dfm <- dfm(N_token)
topN <- head(sort(colSums(N_dfm[ ,!(colnames(N_dfm) %in% c("von"))]), decreasing = TRUE), 15)
...
corpus10N <- corpus(GRW10$Nachname)
...
N_dfm <- dfm(N_token)
topN <- head(sort(colSums(N_dfm[, !(colnames(N_dfm) %in% c("von", "de"))]), decreasing = TRUE), 15)
